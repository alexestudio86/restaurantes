    ...
    try {
        const [blog, posts, pages] = await Promise.all(
            [
                ...
                fetch(`https://www.googleapis.com/blogger/v3/blogs/${import.meta.env.BLOG_ID}/posts?key=${import.meta.env.API_KEY}&fetchImages=true&maxResults=50&fields=items(id,title,labels,content,images)&nextPageToken`).then(r => r.json()),
